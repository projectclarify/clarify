...
class T2TDatagenJobV2(PCMLJob):
    ...
    def launch_shard_parallel_jobs(self, mock=False, dev_max_num_jobs=None):
        ...
        for shard_type in ["train", "dev", "test"]:
            ...
            tf.logging.info("Found %s shards for type %s" %
                            (num_shards, shard_type))

            create_responses = []

            for shard_id in range(num_shards):

                job_name = "%s-%s-%s-%s" % (self.job_name_prefix, shard_type,
                                            shard_id, uid)
...
def main(_):
    ...
    for flag in [
            "data_dir", "num_shards", "shard_id", "problem", "tmp_dir",
            "shard_type", "bigtable_instance", "bigtable_table", "project"
    ]:
