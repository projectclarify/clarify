...
class AVCorrespondenceSample(object):
    ...
    def meta(self, x):
        ...
        for key in [
                ...
                "audio_sample_meta", "audio_keys", "frame_keys",
                "audio_block_meta"
        ]:
            if not key in x:
...
class RawVideoSelection(BigTableSelection):
    ...
    def set_shard_meta(self, shard_meta):
        if not isinstance(shard_meta, VideoShardMeta):
            msg = "Expected type VideoShardMeta, saw {}".format(
                type(shard_meta))
            raise ValueError(msg)
    ...
    def _lookup_all_video_metadata(self, num_shards=1):
        ...
        for shard_meta_key, shard_meta in all_shard_meta.items():
            ...
            end_key = make_video_meta_last_key(self.prefix, shard_id,
                                               num_videos)

            partial_rows = self.table.read_rows(start_key=start_key,
                                                end_key=end_key)
    ...
    """
        while True:
            ...
            try:
                ...
                sampled_video_index = np.random.randint(
                    0, videos_per_sampled_shard)

                tf.logging.debug(
                    "sampled shard index: {}".format(sampled_shard_index))
                tf.logging.debug(
                    "sampled video index: {}".format(sampled_video_index))
    ...
    def _lookup_frame_data(self, frame_keys):
        ...
        for i, frame_key in enumerate(frame_keys):
            ...
            frame_data = row.cells["video_frames"][
                "video_frames".encode()][0].value
    ...
    def sample_av_correspondence_examples(self,
        ...
        while True:
            ...
            f00_, a00_, sampling_meta00 = _sample(v0.video_length,
                                                  v0.audio_length)
            ...
            if keys_only:
                ...
                a00 = self._lookup_audio_data(audio_keys=ka00,
                                              audio_block_meta=abm00)
                a01 = self._lookup_audio_data(audio_keys=ka01,
                                              audio_block_meta=abm01)
                a10 = self._lookup_audio_data(audio_keys=ka10,
                                              audio_block_meta=abm10)
