...
def stage_functions_bundle(gcs_staging_path):
  ...
  with TemporaryDirectory() as tmpdir:
    ...
    for model_filename in ["dev.py",
                           "modality_correspondence.py",
                           "modality_correspondence_utils.py"]:

      model_file_path_src = os.path.join(models_path_src,
                                         model_filename)

      model_file_path_tgt = os.path.join(models_path,
                                         model_filename)
    ...
    main_src = os.path.join(pcml_root, "pcml", "functions",
                            "label_state", "main.py")
...
def _deploy(project_id, region, staging_root, service_account=None,
            collection="users",
