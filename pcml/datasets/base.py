...
class PCMLProblem(problem.Problem):
  ...
  def cbt_generate(self, project, instance, mode, tmp_dir=None, shard_id=-1):
    ...
    target_selection.random_load_from_generator(
      generator=gen, log_every=100, prefix_tag_length=8)
...
class TripletImageProblem(PCMLProblem):
  ...
  def example_reading_spec(self):
    ...
    data_items_to_decoders = {
      ...
      "triplet_code": tf.contrib.slim.tfexample_decoder.Tensor(tensor_key="triplet_code"),
      
      # Dummy
      "targets": tf.contrib.slim.tfexample_decoder.Tensor(tensor_key="triplet_code"),
    }
  ...
  def generate_data(self, data_dir, tmp_dir, task_id=-1):
    ...
    if self.mode == "train":
      ...
      paths = self.test_filepaths(
        data_dir, self.test_shards, shuffled=False)
