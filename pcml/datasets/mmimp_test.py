...
class TestMMIMP(tf.test.TestCase):
  ...
  def test_e2e(self):
    ...
    saved_model_path = os.path.join(export_dir,
                                    tf.gfile.ListDirectory(export_dir)[0])
    ...
    image = tf.reshape(template.fields["image"].mock_one(zeros=True),
                       (4 * 4 * 3,))
        ...
        feature={
            "audio":
                tf.train.Feature(int64_list=tf.train.Int64List(value=[0] *
                                                               100)),
            "video":
                tf.train.Feature(int64_list=tf.train.Int64List(value=video)),
            "image":
                tf.train.Feature(int64_list=tf.train.Int64List(value=image)),
            "eeg":
                tf.train.Feature(int64_list=tf.train.Int64List(value=eeg)),
            "problem_code":
                tf.train.Feature(int64_list=tf.train.Int64List(value=[0] * 1)),
            "targets":
                tf.train.Feature(int64_list=tf.train.Int64List(value=[0] * 12))
        }))
    ...
    def _make_grpc_request(examples,
      ...
      return [{"outputs": outputs[i]} for i in range(len(outputs))]
