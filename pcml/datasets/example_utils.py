...
class ExampleFieldTemplate(object):
    ...
    def __init__(self, modality, vocab_size, space_id, shape, field_type,
                 dtype):
    ...
    def mock(self, zeros=False, batch_size=1):
        ...
        if not isinstance(batch_size, int):
            raise ValueError(
                "batch_size passed to .mock must be of type int, saw %s" %
                batch_size)
...
class ExampleTemplate(object):
    ...
    def mock(self, batch_size=1, zeros=False):
        ...
        for key, field_template in self.fields.items():
            example[key] = field_template.mock(batch_size=batch_size,
                                               zeros=zeros)
        return example
