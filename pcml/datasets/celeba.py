...
class ImageCelebaPcml(ImageCeleba):
    ...
    def preprocess_example(self, example, mode, hparams):
        ...
        example["targets"] = tf.cast(example["targets"],
                                     tf.float32) / tf.constant([256.0])
        # =========================================

        return example

    def example_reading_spec(self):
        data_fields, di2dec = super(ImageCelebaPcml,
...
class CelebaBigTableDev(ImageCelebaPcml):
    ...
    def serving_input_fn(self, hparams, decode_hparams=None, use_tpu=False):
        ...
        if use_tpu:
            padded_shapes = data_reader.pad_for_tpu(dataset.output_shapes,
                                                    hparams, hparams.max_length)
            batch_size = 1 if not decode_hparams else getattr(
                decode_hparams, "batch_size", 1)
            dataset = dataset.padded_batch(batch_size,
                                           padded_shapes,
                                           drop_remainder=False)
            dataset = dataset.map(
                functools.partial(data_reader.pad_batch,
