...
def example_generator(raw_sampler,
    ...
    for raw_sample in raw_sampler:
        ...
        for sample_subtype in raw_sample:
            ...
            video = np.reshape(raw_sample[sample_subtype].video, video_shape)
            ...
            video = augmentation_utils.augment_video(
                video=video, **augmentation_hparams["video"]).astype(np.float32)
            ...
            audio = augmentation_utils.augment_audio(
                ...
                **augmentation_hparams["audio"],
                data_range=[0, 255]).astype(np.float32)
            ...
            audio = standardize_audio_array(audio=audio,
...
class VoxCelebCbt(problem.Problem):
    ...
    * Turned off subsampling operations given currently doing these at the
    ...
    def dataset(self,
        ...
        with tf.device("CPU"):
            ...
            dataset = table.parallel_scan_prefix(
                mode,
                columns=[(selection.column_family, selection.column_qualifier)])
    ...
    def dataset_selection(self, mode):
        ...
        return cbt_utils.TFExampleSelection(project="clarify",
                                            instance="clarify",
    ...
    def eval_metrics(self):
        eval_metrics = [metrics.Metrics.ACC]
        return eval_metrics
...
class VoxCelebSingleFrame(VoxCelebCbt):
    ...
    def preprocess_example(self, example, mode, hparams):
        ...
        example["video"] = (tf.cast(example["video"], tf.float32) -
                            128.0) / 256.0
    ...
    * Turned off subsampling operations given currently doing these at the
    ...
    @property
    def audio_shape(self):
        alen = int(self.audio_source_steps_per_frame /
    ...
    """
        ...
        example = {}
        example["targets"] = tf.slice(example_data, (label_offset,),
                                      (label_length,))
        example["video"] = tf.slice(example_data, (frames_offset,),
                                    (frames_length,))
        example["audio"] = tf.slice(example_data, (audio_offset,),
                                    (audio_length,))
