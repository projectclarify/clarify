...
def conv_pool_3d_block(video,
    ...
    * With default settings returns shape (b, t, h/4, w/4, c).
    ...
    conv_kernel_shape = [
        conv_kernel_depth, conv_kernel_xy, conv_kernel_xy, c, c
    ]
    initializer = tf.truncated_normal_initializer(stddev=0.1)
    conv_kernel = tf.get_variable("conv3d", conv_kernel_shape, dtype,
                                  initializer)
    conv_strides = [1] + conv_strides + [1]

    # Conv 3D
    convolved_video = tf.nn.conv3d(video, conv_kernel, conv_strides,
                                   conv_padding)
