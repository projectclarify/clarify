...
def t2t_preprocess_waveforms(hparams, inputs, scope_name=None, use_bfloat16=False):
...
def resnet_waveforms_v2(hp, waveforms, scope_name, use_bfloat16=False):
  ...
  with tf.variable_scope(scope_name, reuse=tf.AUTO_REUSE):
    ...
    x = t2t_preprocess_waveforms(hp, waveforms, scope_name, use_bfloat16=use_bfloat16)
...
def resnet_frame_stack_v2(hp, stacked_frame_tensor, scope=None,
                          use_bfloat16=False):
...
def conv_pool_3d_block(video, conv_kernel_depth=5, conv_kernel_xy=7,
                       conv_strides=[1,2,2], conv_padding="SAME",
                       maxpool_padding="SAME", maxpool_strides=[1,2,2]):
    """
    ...
    * With default settings returns shape (b, t, h/4, w/4, c).
    ...
    conv_kernel_shape = [conv_kernel_depth, conv_kernel_xy,
                         conv_kernel_xy, c, c]
    initializer = tf.truncated_normal_initializer(stddev=0.1)
    conv_kernel = tf.get_variable("conv3d", conv_kernel_shape,
                                  dtype, initializer)
    conv_strides = [1] + conv_strides + [1]

    # Conv 3D
    convolved_video = tf.nn.conv3d(video,
                                conv_kernel,
                                conv_strides,
                                conv_padding)
