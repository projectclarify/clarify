...
def _testing_run_poll_and_check_job(test_object,
                                    create_response,
                                    expect_in_logs=None):
...
def _testing_run_poll_and_check_tfjob(test_object,
                                      create_response,
                                      expect_in_logs=None):
  ...
  logs = core_api.read_namespaced_pod_log(name=str_if_bytes(
      job_pods["master"][0]),
                                          namespace=created_namespace)
  if expect_in_logs is not None:
  ...
  def test_instantiate(self):
    ...
    cases = [{
        ...
        "expected": {
            'command': ['/bin/sh', '-c'],
            'args': [
                't2t-trainer', '--model=something', '&&', 'echo', 'hworld'
            ],
            'image': 'gcr.io/kubeflow-rl/enhance:0321-2116-e45a',
            'name': 'tensorflow',
            'resources': {
...
class TestAttachedVolume(tf.test.TestCase):
  ...
  def test_instantiate(self):
    ...
    for case in cases:
      ...
      self.assertEqual(object_as_dict(av), case["expected"])
...
class TestLocalSSD(tf.test.TestCase):
  ...
  def test_instantiate(self):
    ...
    for case in cases:
      ...
      self.assertEqual(object_as_dict(v), case["expected"])
...
class TestJob(tf.test.TestCase):
  ...
  def test_instantiates_job(self):
    ...
    for case in cases:
      ...
      self.assertEqual(job.as_dict(), case["expected_dict"])
  ...
  def test_schedules_on_gpu_node_pool(self):
    ...
    class ReportGPU(Job):
      ...
      def __init__(self, *args, **kwargs):
        ...
        command = ["/bin/sh", "-c"]
        command_args = [("export LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:"
                         "/usr/local/nvidia/lib64; echo ${LD_LIBRARY_PATH}; "
                         "export PATH=$PATH:/usr/local/nvidia/bin/; nvidia-smi")
                       ]
        super(ReportGPU, self).__init__(command=command,
...
class TestTFJob(tf.test.TestCase):
  ...
  def test_instantiate_tfjob_replica(self):
    ...
    _ = TFJobReplica(replica_type="MASTER",
                     num_replicas=1,
                     command_args="pwd",
                     ...
                     attached_volumes=[AttachedVolume("nfs-1"),
                                       LocalSSD()],
                     additional_metadata={"master_name": job_name},
                     node_selector={
                         "gpuspernode": 8,
                         "highmem": "true",
                         "preemptible": "true"
  ...
  def test_instantiate_tfjob(self):
    ...
    image = "tensorflow/tensorflow:nightly-gpu"
    train_resources = Resources(limits={"nvidia.com/gpu": 1})
    node_selector = {"cloud.google.com/gke-accelerator": "nvidia-tesla-k80"}
  ...
  def test_can_use_tpu(self):
    ...
    metadata_annotations = {"tf-version.cloud-tpus.google.com": "\"1.12\""}
