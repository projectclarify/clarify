  ...
  """
    ...
    def __init__(self,
        ...
        if not mount_path.startswith("/"):
            raise ValueError("Mount path should start with '/', saw %s" %
                             mount_path)
        ...
        if not isinstance(volume_name, str):
            raise ValueError("Expected string volume_name, saw %s" %
                             volume_name)
        ...
        if ports is not None:
            ...
            for port in ports:
                if not isinstance(port, dict):
                    raise ValueError("ports must be a list of dict.'s, saw %s" %
                                     ports)
...
def job_status_callback(job_response):
    ...
    tf.logging.info(
        "Job %s in namespace %s; uid=%s; succeeded=%s" %
...
class Job(object):
    ...
    """
        ...
        self.apiVersion = (api_version
                           if api_version is not None else "batch/v1")
        ...
        if additional_metadata is not None:
            if not isinstance(additional_metadata, dict):
                raise ValueError(
                    "additional_metadata must be of type dict, saw "
    ...
    def __init__(self,
        ...
        if annotations is not None:
            if not isinstance(annotations, dict):
                raise ValueError("annotations must be dict, saw %s" %
                                 annotations)
...
def log_tfjob_status(tf_job):
    ...
    tf.logging.info(
        ("Job %s in namespace %s; uid=%s; phase=%s, state=%s,",
         ...
         tf_job.get("metadata",
                    {}).get("namespace"), tf_job.get("metadata", {}).get("uid"),
         tf_job.get("status", {}).get("phase"), tf_job.get("status",
                                                           {}).get("state")))
  ...
  """
    ...
    while True:
        ...
        if conditions:
            ...
            if str_if_bytes(last_status) in [
...
class TFJob(Job):
    ...
    def __init__(self,
        ...
        super(TFJob, self).__init__(api_version="%s/%s" %
...
class PCMLJob(Job):
    ...
    def launch_shard_parallel_jobs(self,
        ...
        self.base_command = self.spec["template"]["spec"]["containers"][0].args[
            0]
        ...
        for shard_id in range(num_shards):
            ...
            self.spec["template"]["spec"]["containers"][0].args[
                0] = shard_command
